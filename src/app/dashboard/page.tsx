...
const NewExpense = () => {
  ...
  const getExpenseData = async (email: string) => {
    ...
    try {
      const response = await fetch(
        `/api/chart?type=expenseType&email=${email}`
      );
  ...
  const ComponentsArray = [
    <WeekChart key={"week"} email={email} />,
    <MonthChart key={"month"} email={email} />,
    <ExpenseTypeChart email={email} key={"expense"} />,
  ];
  return (
    <div>
      <Navbar />
            ...
            {photoUrl && (
              <div className="flex space-x-2">
                ...
                <div className="flex flex-col h-full">
                  <h1 className="text-xl leading-[2.6rem] flex-grow my-auto font-semibold tracking-tight">
                    {name}
                  </h1>
          ...
          <div className="flex flex-col py-5">
            ...
            <div className="flex flex-col space-y-2">
              {pieData.map((data, index) => (
                <div
                  className="w-full p-2 flex items-center rounded-xl"
                  key={data.type}
                >
                  <div className="flex w-full">
                      ...
                      style={{
                        backgroundColor:
                          backGroundColorArray[
                            index % backGroundColorArray.length
                          ],
                      }}
                    >
                    ...
                    <div className="w-full p-2 flex justify-between">
                      ...
                      <h1>{converter(data.amount)}</h1>
                    </div>
                  </div>
                </div>
              ))}
              {pieData.length !== undefined && pieData.length == 0 && (
                <div className="text-center p-2">No Activities Yet.</div>
              )}
            </div>
          </div>
        </div>
